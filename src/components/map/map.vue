<template>
    <div class="map-container">
        ...
        <v-enterprisemsg v-if="searchConditions" @listenToConditions="getSearchCondition" :enter-searchvalue="enterPriseValue" :input-value='iptValues' @hoverEnterprise="getHoverEnterPriseBid"></v-enterprisemsg>
        <div id="popup" class="ol-popup">
          <a href="#" id="popup-closer" class="ol-popup-closer"></a>
          <component :is="currentPopup" :popup-properties="properties"></component> 
          <v-key-areapop v-if="keyAreaPopup" :popup-properties="properties"></v-key-areapop>
        </div>
        <!-- 地图右键菜单 -->
  ...
  export default {
    ...
    data () {
      return {
        ...
        saveCurrentPopup: '', //保存地图弹窗组件
        navPath: '',  //保存导航路径
        keyAreaPopup: false,  //片区底图弹窗
        properties: '',
        keyAreaObj: '',  //所选片区类型
    ...
    mounted() {
        ...
        let btnAdd = document.getElementsByClassName('ol-zoom-in');
        btnAdd[0].removeAttribute('title');
        let btnSub = document.getElementsByClassName('ol-zoom-out');
        btnSub[0].removeAttribute('title');

        this.viewExtent = window.openlayerMap.getViewExtentWkt();  //获取当前地图范围

        this.importantArea = window.openlayerMap.addVectorLayer('重点片区',styleKeyArea,11,false);  //重点片区矢量图层
        ...
        this.pointermoveSelect = pointermoveSelect;
        ...
        this.$root.Bus.$on('searchCoditionFirst',(val)=> {   //主页头部第一个搜索
            ...
            if(val.sName === "1") {   //选择为产业用地时
                ...
                this.saveCurrentPopup = 'vLandpop'; 
            }
            if(val.sName === "2") {    //园区搜索
                ...
                this.saveCurrentPopup = 'vHousepop'; 
            }
            if(val.sName === "3") {   //企业搜索
                this.searchConditions = true;
                this.enterParams.names = val.iptName;
                this.getEnterpriseMsgMarksInfor(this.enterParams);
                this.saveCurrentPopup = 'vEnterprisemsgpop';
                publicRequirest.getEnterpriseMsgMarks({names:val.iptName,page:1,count:20}).then(res=> {
                    setTimeout(()=> {   
                        this.enterPriseValue = res.data;
                        this.iptValues = val.iptName;
                        if(this.enterPriseValue.Rows.length === 0) {
                            this.$message({
                                message: '无该条件下数据！！！',
                                type: 'warning',
                                duration: 2000
            ...
            if(val.sName === "4") {   //工业区搜索
                ...
                publicRequirest.getSearchByIndustrailPark(searchVal).then(res=> {
                    if(res.data.code === 200) {
                        ...
                        window.openlayerMap.drawGraphicNoZoom_1('point',this.industrailPark,this.landData);
                    }
                })
                this.saveCurrentPopup = 'vIndustrialparkpop'; 
            }
        })
    ...
    methods: {
        ...
        checkCurrentArea: function() {    //右键地图选择当前片区位置
            ...
            publicRequirest.getCurrentArea(currentPoint).then(res => {
                ...
                res.data.list.forEach(element => {
                    ...
                    sItem.introimg = element.ViewPiece.introimg;
        ...
        getHoverEnterPriseBid(bid) {
            this.pointermoveSelect.featureOverlay_.getSource().clear();
            let allFeatures = this.enterpriseMsg.getSource().getFeatures();
            let _this = this;
            allFeatures.forEach(element=> {
                let ebid = element.getProperties()['bid'];
                if (ebid === bid) {
                    _this.pointermoveSelect.featureOverlay_.getSource().addFeature(element);
                    _this.pointermoveSelect.dispatchEvent('select');
                    return 
                }
            })
        },
    ...
    watch: {
        barmapPath(newVal,oldVal){
            ...
            if(this.barmapPath === "key_area") {   //重点片区
                ...
                this.$nextTick(() => {
                    publicRequirest.getImportKeyAreaInit(this.viewExtent).then(function(res) {
                        let seData = {
                            polyGeoJson: {
                                features: [],
                                type: "FeatureCollection"
                        ...
                        _this.keyAreaData = seData;
                        if(_this.navPath === "industrial_land_1" || _this.navPath === "industrial_house_1" || _this.navPath === "industrial_land_2" || _this.navPath === "industrial_house_2") {
                            window.openlayerMap.changeLayerVisible('重点片区',true);
                        }else{
                            window.openlayerMap.changeLayerVector('重点片区');
                        }
                        window.openlayerMap.drawGraphicNoZoom(null,_this.importantArea,_this.keyAreaData);
                        let imSet = setInterval(function(){
                            if(timer%2 === 0) {
                                window.openlayerMap.changeLayerVisible('重点片区',true);
                            }else {
                                window.openlayerMap.changeLayerVisible('重点片区',false);
                            }
                            timer++;
                            if(timer===7) {
                                clearInterval(imSet);
            ...
            if(this.barmapPath === "guest_park") {  //创客园区
                ...
                this.saveCurrentPopup = 'vGuestparkpop'; 
            }
        ...
        properties(newVal,oldVal) {   //监听地图点、面要素
            if(newVal.geometrypoly) {
                this.currentPopup = '';
                this.keyAreaPopup = true;
            }else{
                this.currentPopup = this.saveCurrentPopup;
                this.keyAreaPopup = false;
            }
        }
    }
...
<style lang=''>
    ...
    .map-container .ol-popup:after, .map-container .ol-popup:before {
        top: 100%;
